...
const DrawerSx = ({ onOpen }: DrawerSxProps) => {
	...
	const [expandedDrawerItem, setExpandedDrawerItem] = useState<MenuItem | null>(
		null
	);
	...
	React.useEffect(() => {
		const handleClickOutsideBox = (event: MouseEvent) => {
			if (
				...
				!boxContainerRef.current.contains(event.target as Node)
			) {
				...
				<>
					...
					{showBox && (
						<BoxSubItems
							...
							ref={boxContainerRef}
						>
							<StyledTitleBoxSubItem variant="subtitle1">
								{expandedDrawerItem ? expandedDrawerItem.label : ""}
							</StyledTitleBoxSubItem>
							<Divider />
							{expandedDrawerItem ? (
								<TreeViewComp menuItem={expandedDrawerItem} />
							) : null}
						</BoxSubItems>
					)}
				</>
